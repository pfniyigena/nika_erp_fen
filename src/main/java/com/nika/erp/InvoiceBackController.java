...
public class InvoiceBackController {
	...
	@Autowired
	private InvoiceBackService invoiceService;
	@Autowired
	...
	public String showInvoiceForm(Model model) {
		InvoiceBack invoice = new InvoiceBack();
		invoice.setInvoiceLines(List.of(new InvoiceLineBack())); // Start with 1 empty line
		model.addAttribute("invoice", invoice);
	...
	@PostMapping("/save")
	public String saveInvoice(@ModelAttribute InvoiceBack invoice) {
	    for (InvoiceLineBack line : invoice.getInvoiceLines()) {
